...
// Check that cache returns an error if external service returns
// an error and validate that the error is the expected
func TestGetPriceFor_ReturnsWellKnowErrorOnServiceError(t *testing.T) {
	mockService := &mockPriceService{
		mockResults: map[string]mockResult{
			"p1": {price: 0, err: fmt.Errorf("some error")},
		},
	...
	if err == nil || !strings.Contains(errMsg, "getting price from service : ") {
		t.Errorf("expected error, got nil")
	}
}
